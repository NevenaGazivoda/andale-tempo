...
export const AtpArticleCard: FC<Props> = ({ article }) => (
    <div className='atp-card'>
        <AtpLink to={`/article/${article.articleCode}`}><img className='atp-card__photo' src={article.images[0].url} alt={article.images[0].title} /></AtpLink>

        <div className='atp-card__info'>
            <AtpLink className='atp-card__link' to={`/article/${article.articleCode}`}>{article.brand}</AtpLink>
